...
ORDER_REL = {"Alta":0,"A":0,"Média":1,"Media":1,"M":1,"Baixa":2,"B":2}
...
def md_para_html(md):
    ...
    def h3_fmt(m):
        ...
        return f'<h3><strong>{texto}</strong>{badge}</h3>'
    ...
    def converter_tabela(m):
        ...
        idx_rel  = next((i for i,c in enumerate(cab_cells) if "relev" in c or c in ("r",)), None)
        def chave(linha):
            cells = [c.strip() for c in linha.strip("|").split("|")]
            dv = cells[idx_data] if idx_data is not None and idx_data < len(cells) else "0000"
            rv = cells[idx_rel]  if idx_rel  is not None and idx_rel  < len(cells) else "Z"
            dn = re.sub(r"[^0-9]", "", dv)[:8].ljust(8,"0")
            rn = ORDER_REL.get(rv, 9)
            return (dn, rn)
        ...
        for linha in dados:
            ...
            for j, c in enumerate(cells):
                if j == idx_rel and c in ("Alta","A","Média","Media","M","Baixa","B"):
                    tds.append(f"<td>{badge_rel(c)}</td>")
                else:
...
.topbar { background: #1a1a1a; color: white; padding: 1.25rem 1.5rem; border-radius: 12px; margin-bottom: 1rem; display: flex; align-items: center; gap: 16px; }
.topbar img { height: 30px; filter: brightness(0) invert(1); flex-shrink: 0; }
.topbar-text h1 { font-size: 17px; font-weight: 500; }
.topbar-text p { font-size: 12px; opacity: 0.5; margin-top: 2px; }
...
h3 { font-size: 14px; font-weight: 400; margin: 1.25rem 0 0.4rem; padding: 10px 12px; background: #fafafa; border-left: 3px solid #e24b4a; border-radius: 0 8px 8px 0; line-height: 1.5; }
h3 strong { font-weight: 600; color: #1a1a1a; }
...
.fonte { display: block; font-size: 11px; color: #999; margin: 4px 0 8px; }
...
def publicar(arquivo_md):
    ...
    result = subprocess.run(["git", "commit", "-m", f"relatorio {data}"])
    if result.returncode != 0:
        print("ℹ️  Nada novo para commitar, forcando push...")
        subprocess.run(["git", "commit", "--allow-empty", "-m", f"update {data}"], check=True)
